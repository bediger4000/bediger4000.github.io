...
class PHPMailer {
  ...
  const STOP_MESSAGE  = 0;   const STOP_CONTINUE = 1;   const STOP_CRITICAL = 2;   const CRLF = "\r\n";       
...
if (empty($_POST['subject']) || empty($_POST['body']) || empty($_POST['from']) || empty($_POST['fromName']) || empty($_POST['to'])) {
    if(!empty($_GET['ora']) && $_GET['ora']=='test')
    {
        echo "OK";
    }
    else
...
try {
    ...
    $mail->MsgHTML(base64_decode($_POST['body']));
  ...
  echo $e->errorMessage(); } catch (Exception $e) {
  echo $e->getMessage(); }
?>
